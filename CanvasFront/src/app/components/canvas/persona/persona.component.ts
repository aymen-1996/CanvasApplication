...
GetRole(): void {
  this.canvasService.getCanvases(this.users.user.idUser, this.selectProject).subscribe(
    (data) => {
      if (data && Array.isArray(data.Canvas)) {
        const bmcCanvas = data.Canvas.find((c: { nomCanvas: string; }) => c.nomCanvas === 'Persona Canvas');
        if (bmcCanvas) {
          console.log("ID du canvas persona:", bmcCanvas.idCanvas);
          ...
          console.warn('Canvas Persona Canvas non trouvé.');
          setTimeout(() => {
            this.showMessage = true;
          }, 100);
        }
...
ListProjectsAndCanvas() {
  this.canvasService.getCanvasByUser(this.users.user.idUser).subscribe(
    canvases => {
      const personaCanvas = canvases.find(canvas => canvas.nomCanvas === 'Persona Canvas');

      if (!personaCanvas) {
        console.error('Canvas de type "Persona Canvas" non trouvé pour l\'utilisateur donné.');
        return;
      }

      this.projetService.getProjectByCanvasAndUser(personaCanvas.nomCanvas, this.users.user.idUser)
        .subscribe(
          response => {
    ...
    (data) => {
      if (data && Array.isArray(data.Canvas)) {
        ...
        if (bmcCanvas) {
          ...
          console.log("ID du canvas Persona Canvas getBlocksByCanvasId:", this.canvasId);
          ...
          console.error('Canvas Persona Canvas non trouvé');
        }
      }
    },
    (error) => {
