...
@Component({
  ...
  styleUrls: ['./vp-canvas.component.css']
})
export class VpCanvasComponent implements OnInit {
  userId!:number
  pendingInvites: any[] = [];
...
//affichage 6 couleurs pour update couleur
    ...
    this.canvasService.getCanvases(this.users.user.idUser, this.selectProject).subscribe(
      (data) => {
        if (data && Array.isArray(data.Canvas)) {
          const vpCanvas = data.Canvas.find((c: { nomCanvas: string; }) => c.nomCanvas === 'VP Canvas');
          
          if (vpCanvas) {
            this.idBloc = vpCanvas.idCanvas;
            console.log("ID du canvas vp getBlocksByCanvasId:", this.idBloc);
            ...
            console.error('Canvas vp non trouvé.');
          }
        } else {
    ...
    if (this.donnees.length < 6) {
      ...
      this.blockService.createDonnees(this.blockId, this.donneesForm.value).subscribe(
        (response) => {
          ...
          this.showAddButton = this.donnees.length < 3; 
        },
        (error) => {
      ...
      const donneeSixieme = this.donnees.splice(2, 1)[0]; 
 ...
 //dans le cas update ticket si je fais entrer l'entrée n'enregistrer dans bd avec ticket
  ...
  onStepChange(event: StepperSelectionEvent): void {
    ...
    if (selectedStepIndex === 6) { // Check if the selected step is "Sommaire"
...
GetRole(): void {
  this.canvasService.getCanvases(this.users.user.idUser, this.selectProject).subscribe(
    (data) => {
      if (data && Array.isArray(data.Canvas)) {
        const bmcCanvas = data.Canvas.find((c: { nomCanvas: string; }) => c.nomCanvas === 'VP Canvas');
        if (bmcCanvas) {
          console.log("ID du canvas vp:", bmcCanvas.idCanvas);
          ...
          console.warn('Canvas VP Canvas non trouvé.');
          setTimeout(() => {
            this.showMessage = true;
          }, 100);
        }
...
calculatePositions(): void {
  ...
  for (let i = 0; i < numPositions; i++) {
    ...
    switch (i) {
      case 0:
        calculatedTop = '125px';
        calculatedLeft = `calc(100% - ${initialLeft}em)`;
        break;
      case 1:
        calculatedTop = `calc(11em + 70px)`;
        calculatedLeft = `calc(100% - 4em)`;
        break;
      case 2:
        calculatedTop = '70px';
        calculatedLeft = `calc(100% - 4em)`;
    ...
    this.positions.push({ top: calculatedTop, left: calculatedLeft });
  }
}
...
listeCanvases(): void {
  this.canvasService.getCanvases(this.users.user.idUser, this.selectProject).subscribe(
    (data) => {
      if (data && Array.isArray(data.Canvas)) {
        const vpCanvas = data.Canvas.find((c: { nomCanvas: string; }) => c.nomCanvas === 'Canvas');
        if (vpCanvas) {
          console.log("ID du canvas vp:", vpCanvas.idCanvas);
          this.idBloc = vpCanvas.idCanvas;
          console.log("idddd", this.idBloc);
          
          this.getBlocksByCanvasId();
        } else {
          console.warn('Canvas vp non trouvé.');
        }
      } else {
        console.error('Format de données inattendu:', data);
      }
...
navigateToVP(project: any): void {
  ...
  const idCanvas = this.getCanvasId(project.idProjet, 'VP Canvas');
  if (idCanvas) {
    this.router.navigateByUrl(`/vp`)
    .then(() => {
      this.updateVPData(idCanvas);
    });
  } else {
...
getCommentCount() {
  this.canvasService.getCanvases(this.users.user.idUser, this.selectProject).subscribe(
    (data) => {
      if (data && Array.isArray(data.Canvas)) {
        ...
        if (bmcCanvas) {
          this.canvasId = bmcCanvas.idCanvas;
          console.log("ID du canvas VP Canvas getBlocksByCanvasId:", this.canvasId);
  ...
  console.error('Canvas VP Canvas non trouvé');
}
}
})
}
